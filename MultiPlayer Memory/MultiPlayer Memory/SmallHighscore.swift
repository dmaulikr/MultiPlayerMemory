...
class SmallHighscore : NSKeyedArchiver, HighscoreProtocol {
    
    static let sharedInstance : SmallHighscore = {
        let instance = SmallHighscore()
        
        if let obj = (NSKeyedUnarchiver.unarchiveObject(withFile: SmallHighscore.ArchiveURL.path) as? [Score]) {
    ...
    static let ArchiveURL = DocumentsDirectory.appendingPathComponent("smallmemscores")
    
    func saveChanges() {
        
        let isSuccessfulSave = NSKeyedArchiver.archiveRootObject(highscores, toFile: SmallHighscore.ArchiveURL.path)
